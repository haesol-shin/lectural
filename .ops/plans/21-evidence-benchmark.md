...
- **Fixture generation** (`tests/fixtures/benchmark/generate.py` + committed generator config/seed + committed output media, EN/KO/mixed):
  - Script (ground truth) authored first; TTS renders audio; PIL renders slide frames from the same script's key terms/numbers.
  - Augraphy degrades slide frames (blur, shadow, perspective, compression-equivalent) at at least two severity levels; audiomentations/ffmpeg inject background noise, silence gaps, and a 360p re-encode variant.
  - One fixture set per language includes: a VTT/json3 caption file feeding `parse_vtt`/`captions_are_usable` directly (usable-caption path) and a deliberately malformed/sparse caption file that must trigger the unusable-caption → STT fallback path — this is the only way to exercise `acquisition.py`'s caption/fallback branches without live YouTube.
...
- **Resource harness** (`scripts/benchmark.py`, new script, local-fixture entry point — distinct from `scripts/perf_smoke.py`'s YouTube-only `run()`):
  ...
  - Execution matrix: `{caption, forced-STT, caption-fallback} × {frame-selection, OCR-on, OCR-off(--skip-ocr)} × {EN, KO, mixed} × {cold, warm} × 3 reps`, plus a small factorial isolating resolution (360p vs 720p source), OCR preprocessing (raw vs. ROI+upscale+Otsu), and STT model size (`small` vs `medium`) on one fixture to attribute resource/quality deltas.
...
2. Fixture generation module + EN/KO/mixed fixture sets (script, TTS audio, PIL slides, Augraphy/audiomentations degraded variants, VTT usable/unusable caption files, GT JSON, one independent-review note).
...
4. `scripts/benchmark.py` harness (extends `StageSampler` usage, execution matrix, cold/warm/3-rep protocol, RTF, storage, `--skip-ocr` pairing, factorial attribution run).
5. `docs/contracts/benchmark.schema.json`, `docs/benchmark_definition.md`, report writer producing `docs/reports/benchmark_<date>.{md,json}`.
...
- A real (opt-in) run of `scripts/benchmark.py` on at least the EN local fixture set, on both an x86_64 host and `pi-server`, producing a schema-valid report with cold/warm/3-rep data and the `--skip-ocr` comparison; recorded under `docs/reports/`.
...
- Whether to commit generated fixture media binaries directly or commit only the generator + locked seed and regenerate in CI/on demand; leaning toward committing (small, short clips) for reviewability, final call during slice 2.
