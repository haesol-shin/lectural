...
- `_ocr_tesseract` uses `run_and_get_multiple_output(image, extensions=["txt", "tsv"], lang="kor+eng")`: returns `(txt_result, confidence)`, confidence from the mean of non-negative `conf` values in the `tsv` result **divided by 100 to normalize Tesseract's 0–100 confidence scale to PaddleOCR's 0–1 scale** before it is stored in the shared `Frame.ocr_confidence` field — without this, a Paddle-calibrated threshold would mark almost all nonzero Tesseract output reliable regardless of actual quality. `txt_result` is returned exactly as before — no reconstruction from TSV rows, so raw text is unmodified.
...
3. **Verify against the real 432.5–444s case, located and confirmed during planning** at `.tmp/youtube-noPuRPDiY6k-20260918-android/` (gitignored, not yet committed — a real `noPuRPDiY6k` extraction, 81 representative frames, `evidence.json` present): `representative_frames` at `432.5s`/`437.0s`/`444.0s` (`frame_00865.png`/`frame_00874.png`/`frame_00888.png`) all hold the same on-screen article/headline screenshot, with PaddleOCR text drifting slightly per frame ("Anthropic CEO says AGl is a marketing te…" / "…AGI is marketing term" / "…AG! isa marketing' te…") — confirming the issue's "similar article frames were duplicated" as OCR-noise-driven, not a genuine content change (the `445.0s` frame, by contrast, has clearly different text and is correctly kept). This is the actual reproduction target, not a guess: run the current pipeline against this frame sequence before the fix (documents today's failure), then after (must merge into one kept frame). **Also** add a synthetic committed fixture reproducing the same failure *signature* (a static screen held across multiple 2fps samples with OCR-noise-driven text drift) to `tests/fixtures/benchmark/` for permanent, redistributable offline regression coverage, since `.tmp/` is gitignored and not distributable (matches the #21 plan's redistributability requirement). Verify `near_duplicate_dropped: true` stays true on the existing shifted-crop fixture and incremental-content fixtures are not regressed by either check.
...
- Explicit pass/fail rule for the benchmark gate, per maintainer decision: for each of three repeated runs, median OCR CPU-seconds and wall-time must not exceed the corresponding #21 baseline-report value by more than **5%**; peak RAM and temporary/final storage must not exceed it by more than **10%** (wider band — RSS peaks carry more run-to-run measurement noise than CPU/wall-time); recall/duplicate-rate/timestamp-integrity/accepted-OCR metrics must **not regress at all** (0% tolerance — these are correctness-style outcomes, not noisy continuous measurements) versus their #21 baseline-report value on the equivalent fixture.
...
## Open decisions
- **Resolved during planning (maintainer decisions, recorded here for traceability):**
  - Tolerance: CPU/wall-time ±5%, peak RAM/storage ±10%, recall/duplicate-rate/timestamp-integrity/accepted-OCR metrics 0% (no regression). See Verification.
  - **Real 432.5–444s case**: located at `.tmp/youtube-noPuRPDiY6k-20260918-android/` — see Chosen approach and Execution slice 3.
