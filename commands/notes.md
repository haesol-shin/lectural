...
1. If `$ARGUMENTS` contains no YouTube URL/ID and no existing local `.mp4`/`.webm`/`.mkv`/`.wav` file, ask the user for a source and stop until they provide one. Do not run anything yet.
