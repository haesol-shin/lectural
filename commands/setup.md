...
1. Run the doctor with the lockfile-consistent invocation: `uv run --project "${CLAUDE_PLUGIN_ROOT}" --extra run lectural doctor --fix` It provisions/verifies the Python run deps (yt-dlp, faster-whisper, opencv, paddleocr, ...), installs/verifies the `yt-dlp` binary, attempts `ffmpeg` best-effort, and checks plugin/hook wiring.
