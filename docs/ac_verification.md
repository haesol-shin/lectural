...
| AC-17 | Full citation gate: `핵심 개념·이론` + `복습 질문` cite `transcript.md#t<id>` that exists in `transcript.md` plus `youtu.be...?t=` within ±1s; `정리 노트` is citation-exempt | `tests/test_notes.py`; `tests/test_redteam_notes.py` citation/deeplink tests; `tests/test_redteam_notes_contract.py::test_coverage_contract_rejects_dangling_anchor_and_missing_youtube`, `::test_youtube_seconds_tolerance_is_one_second` | unit ✅ |
