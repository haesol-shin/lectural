...
| AC-10 | SKILL.md + scripts + references; callable in Claude/Codex | `skills/lectural/SKILL.md` + `references/` + `commands/` (`/lectural:notes`, `/lectural:setup`); `hooks/hooks.json` Stop hook; dev load via `claude --plugin-dir .`; `uv run python -m lectural.cli --help` | artifact + CLI ✅ |
